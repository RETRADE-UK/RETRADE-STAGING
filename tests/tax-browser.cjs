...
(async()=>{
 ...
 try{
  ...
  assert.equal(await page.locator('.tax-method-card').count(),0);
  assert.equal(await page.evaluate(()=>_taxExportData.method),'Actual expenses');
  assert.equal(await page.evaluate(()=>_taxExportData.tradingAllowanceUsed),0);
  ...
  await mobile.page.evaluate(()=>renderTax());
