...
(async()=>{
 ...
 try{
  ...
  await page.locator('.ip-owner-details summary').tap();
  ...
  await page.locator('#ip-partner-percent').fill('30');
  assert((await page.locator('#ip-cost-preview').innerText()).includes('Preview ·'));
  assert.equal(await page.evaluate(()=>DB['SEP-26'][0].accountSplitPercent),50,'Preview does not persist edits');
  await page.locator('.ip-cost-form button[type=submit]').tap();
